  ...
  try {
    ...
    for (const m of live ?? []) {
      ...
      const { data: snap } = await supabase
        .from('match_score_snapshots').select('*').eq('match_id', m.id).maybeSingle();
      ...
      await supabase.from('match_score_snapshots').upsert({
        match_id: m.id, home_score: home, away_score: away, status: m.status, updated_at: new Date().toISOString(),
      }, { onConflict: 'match_id' });
      ...
      if (successUsers.size) {
        await supabase.from('notification_log').insert(
          [...successUsers].map((uid) => ({
            user_id: uid, type: 'goal', match_id: m.id, dedupe_key: dedupeKey, title, body,
          })),
        );

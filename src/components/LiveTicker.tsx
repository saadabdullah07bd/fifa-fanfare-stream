    ...
    <section className="mx-auto max-w-7xl px-4 pb-6">
      ...
      {(upcoming.length > 0 || finished.length > 0) && (
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          {upcoming.length > 0 && (
            <MiniList title="Kicking off soon" items={upcoming} showTime />
          )}
          {finished.length > 0 && (
            <MiniList title="Full time today" items={finished} />
          )}
        </div>
      )}
    </section>
...
function LiveCard({ m }: { m: LiveMatch }) {
  ...
  return (
    <article className="live-shimmer rounded-xl border border-primary/40 bg-card/70 p-4 shadow-lg">

...
function LiveCard({ m }: { m: LiveMatch }) {
  const min =
    m.status === "PAUSED"
      ? "HT"
      : m.minute
        ? `${m.minute}${m.injury_time ? `+${m.injury_time}` : ""}'`
        : "LIVE";
  return (
    <Link
      to={wcHref(m)}
      className="live-shimmer block rounded-xl border border-primary/40 bg-card/70 p-4 shadow-lg transition hover:-translate-y-0.5 hover:border-primary"
    >
      ...
      <div className="mt-3 space-y-2">
        <TeamRow
          ...
          tla={m.home.tla}
          crest={m.home.crest}
          score={m.score.full.home}
        />
        <TeamRow
          name={m.away.name}
          tla={m.away.tla}
          crest={m.away.crest}
          score={m.score.full.away}
        />
      </div>
    </Link>
  );

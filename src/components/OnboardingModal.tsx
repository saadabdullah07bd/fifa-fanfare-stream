        ...
        >
          ...
          >
            <div className="relative bg-gradient-to-br from-primary/25 via-accent/15 to-transparent p-6">
              ...
              <h2 className="display mt-1 text-3xl tracking-wider sm:text-4xl">Who are you backing?</h2>
              <p className="mt-1 text-sm text-muted-foreground">
                Pick your World Cup 2026 team — we'll surface their fixtures, results and group table on your homepage.
              </p>
            </div>
            ...
            <div className="p-5">
              ...
              <div className="mt-4 max-h-[26rem] space-y-4 overflow-y-auto pr-1">
                {teams.length === 0 && <p className="p-3 text-xs text-muted-foreground">Loading teams…</p>}
                {grouped.map(([label, ts]) => (
                  <div key={label}>
                    <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-primary">{label}</p>
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                      {ts.map((t) => (
                        <button key={t.code} onClick={() => pick(t)} disabled={saving}
                          ...
                          {t.flag_url ? (
                            <img src={t.flag_url} alt="" width={28} height={20} loading="lazy"
                              className="h-5 w-7 rounded-sm object-cover shadow" />
                          ) : (
                            <span className="grid h-5 w-7 place-items-center rounded-sm bg-muted text-[9px] font-bold">{t.code}</span>
                          )}
                          <span className="min-w-0 flex-1 truncate text-sm font-semibold group-hover:text-primary">{t.name}</span>
                          <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{t.code}</span>
                        </button>
                      ))}
                    </div>
                  </div>

...
export default function Auth() {
  ...
  const from =
    typeof fromState === "string" && fromState.startsWith("/") && fromState !== "/auth"
      ? fromState
      : "/live-tv";
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-24">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Sign in with Google to access Pitch26 — save favorite teams and stream live matches.
      </p>
      ...
      */}
      <button
        type="button"
        onClick={() => {
          window.sessionStorage.setItem("postAuthRedirect", from);
          google();
        }}
      ...
      >
        <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden="true">
          <path
            fill="#4285F4"
            d="M23.49 12.27c0-.79-.07-1.54-.19-2.27H12v4.51h6.46c-.28 1.48-1.13 2.74-2.4 3.58v2.98h3.87c2.26-2.09 3.56-5.17 3.56-8.8z"
          />
          <path
            fill="#34A853"
            d="M12 24c3.24 0 5.95-1.08 7.93-2.91l-3.87-2.98c-1.07.72-2.44 1.16-4.06 1.16-3.12 0-5.77-2.11-6.72-4.96H1.29v3.09C3.26 21.3 7.31 24 12 24z"
          />
          <path
            fill="#FBBC05"
            d="M5.28 14.31c-.24-.72-.38-1.49-.38-2.31s.14-1.59.38-2.31V6.6H1.29A11.996 11.996 0 000 12c0 1.94.46 3.78 1.29 5.4l3.99-3.09z"
          />
          <path
            fill="#EA4335"
            d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.42-3.42C17.95 1.19 15.24 0 12 0 7.31 0 3.26 2.7 1.29 6.6l3.99 3.09C6.23 6.86 8.88 4.75 12 4.75z"
          />
        </svg>
      ...
      <p className="mt-6 text-center text-xs text-muted-foreground">
        We only use your Google account to sign you in. We request access to your name, email

...
import { saveXtreamConfig, getXtreamConfig, refreshChannels } from "@/lib/xtream.functions";
...
function Settings() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12">
      ...
      <p className="mt-2 text-muted-foreground">Connect your Xtream Codes server. Credentials stay on the server — we never expose them in the browser.</p>

      {cfg && (
        <div className="mt-6 rounded-lg border border-border bg-card/40 p-4">
          <p className="text-xs uppercase tracking-wider text-primary">Connected</p>
      ...
      <form onSubmit={(e) => { e.preventDefault(); save.mutate(); }} className="mt-8 space-y-3">
        <h2 className="display text-2xl">{cfg ? "Update credentials" : "Add Xtream server"}</h2>
        ...
        <input required type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)}
          className="w-full rounded-md border border-border bg-input px-3 py-3 text-sm" />
        <button disabled={save.isPending} className="rounded-md bg-primary px-4 py-3 text-sm font-bold uppercase tracking-wider text-primary-foreground disabled:opacity-50">
          {save.isPending ? "Saving…" : "Save server"}
        </button>
      </form>

      <button onClick={async () => { await supabase.auth.signOut(); navigate({ to: "/" }); }}
        className="mt-10 text-xs uppercase tracking-wider text-muted-foreground hover:text-destructive">
        Sign out
      </button>
    </div>
  );
